...
sql.getPlayer = async function (id) {
    const query = sqlstring.format('SELECT * FROM players WHERE id=?;', [id]);
    return sql.runQuery(query);
};
...
sql.deleteGameAndPlayers = async function (game) {
    let query = sqlstring.format('DELETE FROM players WHERE game = ?;', [game]);
    await sql.runQuery(query);

    query = sqlstring.format('DELETE FROM games WHERE id = ?;', [game]);
    return sql.runQuery(query);
};
